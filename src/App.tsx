...
import processBoxes from "./box";
import createHierarchyTree from "./hierarchy";
...
function App() {
      ...
      if (hasChanges) {
        startTransition(() => {
          setTransformationString(
            `translate(${currentTranslationX.current}px,${currentTranslationY.current}px) scale(${currentScale.current})`
          );
        });
      }
    },
  ...
  const wheeled = useCallback((event: WheelEvent) => {
    ...
    const scaleMultiplier =
      -deltaY * (1 === deltaMode ? 0.05 : deltaMode ? 1 : 5e-4);
    zoomBehavior.current!.scaleTo(
      selection.current!,
      currentScale.current + scaleMultiplier,
      [mouseX, mouseY]
    );
  }, []);
  ...
  const setTranslateExtent = useCallback(() => {
    ...
    zoomBehavior.current?.translateExtent([
      [
        -translationXLimit - shiftConstant,
        -translationYLimit + area.current.minY - shiftConstant,
      ],
      [
  ...
  const centerRoot = useCallback(() => {
    ...
    selection.current
      ...
      .call(
        ...
        d3.zoomIdentity
          .scale(1)
          .translate(viewportWidth / 5, viewportHeight / 2)
      );
  }, []);
  ...
  return (
    <div className="zoom" ref={zoomDomRef}>
      <div className="diagram" style={{ transform: transformationString }}>
        <svg className="svg-paths">
          {diagram.paths?.map((path, idx) => (
            <path d={path} key={`path-${idx}`} />
          ))}
        </svg>
        ...
        {diagram.blocks?.map((block, idx) => (
            <div
              ...
              key={`block-${idx}`}
            >
              <div className="block" data-type={block.data.type}></div>
            </div>
          ))}
      </div>
    </div>
